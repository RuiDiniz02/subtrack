...
export default function SubscriptionModal({ onClose, onSave, initialData }: SubscriptionModalProps) {
    ...
    const [category, setCategory] = useState(categories[0]); // Novo estado
    ...
    const modalContent = (
        ...
        >
            ...
            >
                <button onClick={onClose} className="absolute top-3 right-3 text-text-light hover:text-text-main">
                    <XIcon />
                </button>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-4 pt-4">
                        <button type="button" onClick={onClose} className="py-2 px-4 bg-secondary hover:bg-secondary/80 rounded-lg font-semibold transition-colors text-text-light">Cancelar</button>
                        <button type="submit" className="py-2 px-4 bg-action hover:opacity-90 rounded-lg font-bold text-white transition-colors">
